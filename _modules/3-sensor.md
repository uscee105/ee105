---
title: Module 3 - Sensors
---

Oct 15
: [Sensors 1]({{site.url}}{{site.baseurl}}/assets/module-3-sensor/mod-3-lecture-13-sensors-1.pdf)
  : [**Lab**{: .label .label-black} Temperature sensor]({{site.url}}{{site.baseurl}}/assets/module-3-sensor/mod-3-lecture-13-temperature-sensor.zip)
: **Weekly Quiz**{: .label .label-orange }

Oct 20
: [Sensors 2]({{site.url}}{{site.baseurl}}/assets/module-3-sensor/mod-3-lecture-14-sensors-2.pdf)
  : **Final project teaming**{: .label .label-black} Groups of 5 students

Oct 22
: [**Lab**{: .label .label-purple } Oximeter data collection and processing]({{site.url}}{{site.baseurl}}/assets/module-3-sensor/mod-3-oximeter-lab.pdf)
: [**Lab**{: .label .label-purple } Lab files]({{site.url}}{{site.baseurl}}/assets/module-3-sensor/mod-3-oximeter-lab.zip)
: [**Lab checkoff**{: .label .label-purple } Form](https://docs.google.com/forms/d/e/1FAIpQLSdJVwg85IJBetfICou6neE-N0po9H496aS-cujr0vhczSuMQg/viewform)
: **Weekly Quiz**{: .label .label-orange }

Oct 27
: [Sensors 3]({{site.url}}{{site.baseurl}}/assets/module-3-sensor/mod-3-lecture-16-sensors-3.pdf)
: **Final project plan due**{: .label .label-red }
